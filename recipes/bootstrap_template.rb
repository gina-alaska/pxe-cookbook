...
remote_file "#{node['pxe']['dir']}/chef-full.erb" do
...
ruby_block "template url" do
  block do
    ...
    sed += " #{node['pxe']['dir']}/chef-full.erb"
    Chef::Log.debug sed
    cmd = Mixlib::ShellOut.new(sed)
    output = cmd.run_command
    Chef::Log.debug output.stdout
    nodetemplate = File.new("#{node['pxe']['dir']}/#{node['hostname']}.erb", "w+")
    nodetemplate.puts output.stdout
    nodetemplate.chmod(0644)
  end
  action :nothing
  subscribes :create, resources("remote_file[#{node['pxe']['dir']}/chef-full.erb]")
end

#write out a new install.sh
remote_file "#{node['pxe']['dir']}/original-install.sh" do
...
ruby_block "install.sh url" do
  block do
    ...
    sed += " #{node['pxe']['dir']}/original-install.sh"
    Chef::Log.debug sed
    cmd = Mixlib::ShellOut.new(sed)
    output = cmd.run_command
    Chef::Log.debug output.stdout
    nodeinstall = File.new("#{node['pxe']['dir']}/#{node['hostname']}-install.sh", "w+")
    nodeinstall.puts output.stdout
    nodeinstall.chmod(0644)
  end
  action :nothing
  subscribes :create, resources("remote_file[#{node['pxe']['dir']}/original-install.sh]")
end

#straighten up symlinks for downloading from install.sh
ruby_block "create symlinks that match up with the urls" do
  block do
    Dir.glob("#{node['pxe']['dir']}/opscode-full-stack/*").each do |distro|
